...
- **`./scripts/check.sh --fast` doit être vert à la fin de chaque tâche.**
  Le ratchet `.tripwire-testcount` compte les **assertions textuelles**
  (`grep -rho TEST_ASSERT test/`, cf. `scripts/check.sh:38`) — 463 aujourd'hui,
  pour 125 fonctions de test. **Ne jamais écrire cette valeur à la main** :
  `check.sh` la relève lui-même quand elle monte (`check.sh:223-224`) ; il
  suffit de committer le fichier tel qu'il l'a écrit. Une baisse reste refusée.
...
- [ ] **Étape 2 : ne PAS conditionner le brochage SD**

Le brochage `BOARD_SD_*` et ses six `_Static_assert` restent **inconditionnels**,
et `main/storage/sd_card.c` reste compilé sur les trois cartes. C'est délibéré,
et l'inverse casserait le build : `main/usb/msc_disk.c` appelle `sd_present()`,
`sd_sector_count()`, `sd_sector_size()` et `sd_read_sectors()` (lignes 78-145)
et il est compilé partout, donc `sd_card.c` doit compiler, donc les macros
doivent exister.

Garder ce code compilé a un second mérite : `usb mode storage` reste
atteignable par la console sur la carte-clé. Vérifié sur matériel le
2026-08-16 — sans carte insérée, l'hôte attache puis annonce
`[sda] Media removed, stopped polling`, sans blocage ni erreur d'E/S.

`BOARD_HAS_SD` ne gouverne donc que **deux** choses, toutes deux à l'étape 6 :
le sondage au démarrage dans `main.c`, et la commande console `sd`. C'est là
qu'étaient les onze secondes, et c'est tout ce qu'on cherchait à supprimer.

Rien à faire dans cette étape hormis en prendre acte : ne pas toucher au bloc
microSD de `board_common.h`.
...
```
Attendu : vert. `check.sh` relève lui-même `.tripwire-testcount` et le dit
(`ratchet: 463 -> N tests … à committer`). Committer le fichier tel quel.
...
Lancer `./scripts/check.sh --fast` : il relève `.tripwire-testcount` tout seul.
Committer le fichier tel qu'il l'a écrit — ne jamais saisir la valeur à la main.
...
Dans `main/CMakeLists.txt`, ajouter `"hmi/hmi.c"` à `srcs` et
`espressif__led_strip` à `REQUIRES`. Ne PAS ajouter `"hmi"` à `INCLUDE_DIRS` :
`#include "hmi/…"` résout déjà par `.`, et l'ajouter suggérerait à tort que ces
en-têtes s'incluent à plat.
...
**Ratchet.** Le compteur suit les assertions textuelles, pas les fonctions de
test (`scripts/check.sh:38`). Aucune tâche n'écrit sa valeur à la main :
`check.sh` la relève, la tâche commite le fichier.
